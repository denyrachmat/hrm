...
<div class="modal fade" id="modalApproved{{ $model->id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('leave-requests.approved') }}" method="POST">
                @csrf
                @method('POST')
                <div class="modal-body">
                    <div class="form-group">
                        <label for="exampleFormControlTextarea1">Start Date</label>
                        <input type="date" class="form-control" value="{{ \Carbon\Carbon::parse($model->start_date)->format('Y-m-d') }}" readonly>
                    </div>
                    <div class="form-group">
                        <label for="exampleFormControlTextarea1">End Date</label>
                        <input type="date" class="form-control" value="{{ \Carbon\Carbon::parse($model->end_date)->format('Y-m-d') }}" readonly>
                    </div>
                    <div class="form-group">
...
<div class="modal fade" id="modalRejected{{ $model->id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('leave-requests.rejected') }}" method="POST">
                @csrf
                @method('POST')
                <div class="modal-body">
                    <div class="form-group">
...
<td>
    ...
    <a href="{{ route('leave-requests.show', $model->id) }}" class="btn btn-outline-success btn-sm">
        <i class="fa fa-eye"></i>
    </a>
    @endcan

    @can('leave request edit')
        <a href="{{ route('leave-requests.edit', $model->id) }}" class="btn btn-outline-primary btn-sm">
            <i class="fa fa-pencil-alt"></i>
        </a>
    @endcan

    @can('leave request delete')
        <form action="{{ route('leave-requests.destroy', $model->id) }}" method="post" class="d-inline"
